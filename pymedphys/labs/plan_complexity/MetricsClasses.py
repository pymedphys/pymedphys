...
import MLCdata
from math import pi
from PlanOperations import getMLCdata, getBeamType
...
class ModulationComplexity(Metrics): 
    ...
    def calcLSV(self,left,right,y1,y2,indices):
        ...
        for l in range(len(right)-1):
            ...
            if math.isnan(leafDiff): continue
    ...
    def calcApperturePerim(self,indices,A,B,y1,y2):
        ...
        for i in range(0,60):
            ...
            if i == 0: #no leaf above so only checking one side
                if A[i+1] < A[i]: edgesSeg = edgesSeg + abs(A[i+1] - A[i])                        
                if B[i+1] > B[i]: edgesSeg = edgesSeg + abs(B[i+1] - B[i])
                continue
