...
plt.title("Gamma ({0}) | Percent Pass: {1:.2f} %".format(
    threshold_string, pass_ratio*100))
...
(z_eval, y_eval, x_eval), dose_evaluation = zyx_and_dose_from_dataset(
    evaluation)


lower_dose_cutoff = (gamma_options['lower_percent_dose_cutoff'] /  # type: ignore
                     100 * np.max(dose_reference))
...
for z_val, eval_slice, ref_slice, diff, gamma_slice in zip(
    ...
    fig.savefig(os.path.join(
        output_dir, 'slice_{:+06.1f}.gamma.png'.format(z_val)))
