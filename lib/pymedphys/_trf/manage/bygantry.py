...
def calc_normalisation(mosaiq_delivery_data):
    all_gantry_angles = mosaiq_delivery_data.metersetmap
...
def calc_logfile_metersetmap_bygantry(
    index, config, logfile_group, gantry_angle, grid_resolution=1
):
    logfile_metersetmap = None
    ...
    for filehash in logfile_group:
        ...
        a_logfile_metersetmap = [
            pymedphys.metersetmap.grid(grid_resolution=grid_resolution),
            logfile_delivery_data.metersetmap(
                gantry_angle, grid_resolution=grid_resolution
            ),
        ]

        if logfile_metersetmap is None:
            logfile_metersetmap = a_logfile_metersetmap
        else:
            assert np.all(logfile_metersetmap[0] == a_logfile_metersetmap[0])
            assert np.all(logfile_metersetmap[1] == a_logfile_metersetmap[1])
            logfile_metersetmap[2] += a_logfile_metersetmap[2]

    return logfile_metersetmap
...
def compare_logfile_group_bygantry(
    ...
    mosaiq_metersetmap = [
        pymedphys.metersetmap.grid(grid_resolution=grid_resolution),
        mosaiq_delivery_data.metersetmap(gantry_angle, grid_resolution=grid_resolution),
    ]

    normalisation = calc_normalisation(mosaiq_delivery_data)

    logfile_metersetmap = calc_logfile_metersetmap_bygantry(
        index, config, logfile_group, gantry_angle
    )

    grid_xx = logfile_metersetmap[0]
    grid_yy = logfile_metersetmap[1]
    assert np.all(grid_xx == mosaiq_metersetmap[0])
    assert np.all(grid_yy == mosaiq_metersetmap[1])

    comparison = calc_comparison(
        logfile_metersetmap[2], mosaiq_metersetmap[2], normalisation
    )

    print(comparison)
    plot_results(grid_xx, grid_yy, logfile_metersetmap[2], mosaiq_metersetmap[2])
...
def get_logfile_metersetmap_bygantry(
            ...
            for mosaiq_gantry_angle in mosaiq_gantry_angles:
                ...
                if num_control_points > 0:
                    metersetmap = [
                        ...
                        delivery_data[file_hash][mosaiq_gantry_angle].metersetmap(),
                    ]

                    if (
                        mosaiq_gantry_angle
                        not in logfile_metersetmap_bygantry[logfile_group]
                    ):
                        logfile_metersetmap_bygantry[logfile_group][
                            mosaiq_gantry_angle
                        ] = list(metersetmap)
                    else:
                        assert np.all(
    ...
    return logfile_metersetmap_bygantry
...
def get_mosaiq_metersetmap_bygantry(mosaiq_delivery_data_bygantry):
    mosaiq_metersetmap_bygantry = dict()
    mosaiq_gantry_angles = mosaiq_delivery_data_bygantry.keys()

    for mosaiq_gantry_angle in mosaiq_gantry_angles:
        metersetmap = [
            pymedphys.metersetmap.grid(),
            mosaiq_delivery_data_bygantry[mosaiq_gantry_angle].metersetmap(),
        ]
        mosaiq_metersetmap_bygantry[mosaiq_gantry_angle] = metersetmap

    return mosaiq_metersetmap_bygantry


def get_comparison_results(
    mosaiq_metersetmap_bygantry, logfile_metersetmap_bygantry, normalisation
):
    comparison_results = dict()
    mosaiq_gantry_angles = mosaiq_metersetmap_bygantry.keys()
    logfile_groups = list(logfile_metersetmap_bygantry.keys())

    for mosaiq_gantry_angle in mosaiq_gantry_angles:
        comparison_results[mosaiq_gantry_angle] = dict()
        comparison_results[mosaiq_gantry_angle]["comparisons"] = {}

        grid_xx = mosaiq_metersetmap_bygantry[mosaiq_gantry_angle][0]
        grid_yy = mosaiq_metersetmap_bygantry[mosaiq_gantry_angle][1]
        mosaiq_metersetmap = mosaiq_metersetmap_bygantry[mosaiq_gantry_angle][2]

        for logfile_group in logfile_groups:
            assert np.all(
                grid_xx
                == logfile_metersetmap_bygantry[logfile_group][mosaiq_gantry_angle][0]
            )
            assert np.all(
                grid_yy
                == logfile_metersetmap_bygantry[logfile_group][mosaiq_gantry_angle][1]
            )

            logfile_metersetmap = logfile_metersetmap_bygantry[logfile_group][
                mosaiq_gantry_angle
            ][2]

            comparison = calc_comparison(
                logfile_metersetmap, mosaiq_metersetmap, normalisation
            )
            comparison_results[mosaiq_gantry_angle]["comparisons"][
                logfile_group
            ] = comparison
...
def get_metersetmaps_for_file_hashes(index, config, cursor, file_hashes):
    field_ids = {
    ...
    logfile_metersetmap_bygantry = get_logfile_metersetmap_bygantry(
        logfile_groups, mosaiq_gantry_angles, logfile_delivery_data_bygantry
    )
    mosaiq_delivery_data_bygantry = get_mosaiq_delivery_data_bygantry(
        mosaiq_delivery_data
    )
    mosaiq_metersetmap_bygantry = get_mosaiq_metersetmap_bygantry(
        mosaiq_delivery_data_bygantry
    )

    normalisation = calc_normalisation(mosaiq_delivery_data)

    return (mosaiq_metersetmap_bygantry, logfile_metersetmap_bygantry, normalisation)


def get_metersetmaps_for_field_id(
    index, config, cursor, field_id, field_id_grouped_hashes
):
    file_hashes = np.array(field_id_grouped_hashes[field_id])
    metersetmaps = get_metersetmaps_for_file_hashes(index, config, cursor, file_hashes)
...
def get_comparisons_byfield(index, config, cursor, field_ids, field_id_grouped_hashes):
    ...
    for field_id in field_ids:
        metersetmaps = get_metersetmaps_for_field_id(
            index, config, cursor, field_id, field_id_grouped_hashes
        )
        comparison_results = get_comparison_results(*metersetmaps)
