...
- **[#2065](https://github.com/pymedphys/pymedphys/pull/2065), M0 experimental pseudonymisation limitation warnings.** Open, based on #2061. Implements the D-023 limitation warnings for experimental pseudonymisation. `pseudonymise`, `get_default_pseudonymisation_keywords`, and `is_valid_strategy_for_keywords` emit `PseudonymisationLimitationWarning`, a `UserWarning` rather than a deprecation, from the public `pymedphys.experimental.pseudonymisation` interface, so the private implementation's internal calls do not repeat it. `pymedphys experimental dicom pseudonymise` prints the same notice on standard error, and the app shows it as a banner. The wrappers keep the public module and name, so they still pickle by reference. `pseudonymisation_dispatch`, a dictionary of strategy functions, is re-exported without a warning. The notice states the unkeyed hashing (original UIDs re-link records; small-range values such as weight are recoverable from the output alone), the single date offset, and the retained preamble and File Meta SOP Instance UID, and links the background page. `pseudonymise()` now keeps Patient's Sex, which a `list("PatientSex")` bug had hashed into a value that is not permitted; the retained value is a quasi-identifier, and the command still hashes it by default. Ten tests (written first) cover one non-deprecation warning per public call, pickling by reference, a silent private implementation, the Patient's Sex regression, the CLI notice, and the app banner before the uploader; pre-commit, pyright, pylint, and a Sphinx build with warnings as errors pass. Download-dependent tests rely on CI. Known defects left in the legacy code (reproduced): an age with unit `D` raises `UnboundLocalError`; a non-ASCII name raises `UnicodeEncodeError`; ages of 20 or under are jittered at random on every call, so one patient's studies can disagree; a pseudonymised name exceeds the 64-character limit on a person name component group; and about 2% of decimal strings (for example a weight of 31.8) raise `ValueError`, because a hash prefix with a leading zero yields fewer digits than the value.
