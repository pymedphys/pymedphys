...
- **[#2062](https://github.com/pymedphys/pymedphys/pull/2062), M0 legacy diagnostics.** Open, based on #2061. Removes identifying values and paths from explicit application logging and progress output, with capture tests. Eight canary tests capture both standard streams and the records that reach the root logger at DEBUG level, and pass alongside pre-commit, type/lint checks, and an app-render check. The existing download-dependent tests were blocked locally by Zenodo access and rely on CI. Two disclosure channels remain, tracked in the near-term queue: re-raised exceptions, whose messages can contain paths and values; and pydicom validation messages, which quote invalid values and reach both Python warnings and the `pydicom` logger (propagating to the root logger). A strict expected-failure test documents the second channel with a malformed Study Time during pseudonymisation. This is a partial implementation of D-020, not a deprecation change or a claim that every diagnostic channel is sanitised.
