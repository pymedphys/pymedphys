...
- **[#2065](https://github.com/pymedphys/pymedphys/pull/2065), M0 experimental pseudonymisation limitation warnings.** Open, based on #2061. Records D-023, which replaces D-009: no interface is deprecated before its replacement is released. `pseudonymise`, `get_default_pseudonymisation_keywords`, and `is_valid_strategy_for_keywords` emit `PseudonymisationLimitationWarning`, a `UserWarning` rather than a deprecation, from the public `pymedphys.experimental.pseudonymisation` interface, so the private implementation's internal calls do not repeat it. `pymedphys experimental dicom pseudonymise` prints the same notice on standard error, and the app shows it as a banner. The notice states the unkeyed hashing, the single date offset, and the retained preamble and File Meta SOP Instance UID, and links the background page. `pseudonymise()` now keeps Patient's Sex, which a `list("PatientSex")` bug had hashed into an invalid code string. Seven tests (written first) cover one non-deprecation warning per public call, a silent private implementation, the Patient's Sex regression, the CLI notice, and the app banner; pre-commit, pyright, pylint, and a Sphinx build with warnings as errors pass. Download-dependent tests rely on CI. Known defects left in the legacy code (reproduced): an age with unit `D` raises `UnboundLocalError`; a non-ASCII name raises `UnicodeEncodeError`; ages of 20 or under are jittered at random on every call, so one patient's studies can disagree; and a pseudonymised name exceeds the 64-character limit on a person name component group.
...
- **Consequences.** The legacy API names will be deprecated and removed according to D-023, and only after their replacements are usable. This design PR does not change their runtime status. Documentation explains key custody and recipient context instead of promising anonymity.
...
- **Decision.** Disclose the known security limitations of experimental pseudonymisation immediately, at its library, command-line, and app entry points, with warnings that are not deprecations and make no removal promise: a `UserWarning` subclass in the library, a standard-error notice on the command line, and a banner in the app. Deprecate a legacy interface only when a released replacement covers its use and migration guidance ships (for the library and command-line interfaces, M3 to M5). From then, keep each legacy interface for at least one full minor release with deprecation warnings before removing it. Remove `pymedphys.dicom.anonymise` and experimental pseudonymisation together in the removal release. Record exact versions in the release notes when scheduled.
...
- **Validation.** Tests check that the limitation warnings are not `DeprecationWarning`s and mention no removal, that each public call warns once, and that the command-line and app notices appear. When deprecation starts, tests check the deprecation warnings and that the migration guide is published.
...
| Experimental pseudonymisation security-limitation warnings | Under review in #2065, based on #2061. Covers library, CLI, and app entry points. Not a deprecation: deprecation waits for a released replacement (D-023). |
...
- **Release versions:** record the release that first carries the experimental limitation warnings, the replacement release that starts the deprecation window for both legacy interfaces, and the removal release, when scheduled under D-023. No version or deprecation is introduced by this documentation PR.
