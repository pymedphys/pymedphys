...
- **[#2065](https://github.com/pymedphys/pymedphys/pull/2065), M0 experimental pseudonymisation deprecation.** Open, based on #2061. `pseudonymise`, `get_default_pseudonymisation_keywords`, and `is_valid_strategy_for_keywords` emit a `DeprecationWarning` from the public `pymedphys.experimental.pseudonymisation` interface, so the private implementation's internal calls do not repeat it; `pymedphys experimental dicom pseudonymise` prints the notice on standard error because `DeprecationWarning` is hidden by default; and the app shows a warning banner. The notice states the security limitations and links the background page. `pseudonymise()` now keeps Patient's Sex, which a `list("PatientSex")` bug had hashed into an invalid code string. Seven tests (written first) cover one warning per public call, a silent private implementation, the Patient's Sex regression, the CLI notice, and the app banner; pre-commit, pyright, pylint, the import check, and a Sphinx build with warnings as errors pass. Download-dependent tests rely on CI. No version is scheduled: the warning ships in the first release that includes this PR, and D-009 versions are recorded then. Removal is separate, later work. Known defects left in the deprecated code (reproduced): an age with unit `D` raises `UnboundLocalError`; a non-ASCII name raises `UnicodeEncodeError`; ages of 20 or under are jittered at random on every call, so one patient's studies can disagree; and a pseudonymised name exceeds the 64-character limit on a person name component group.
...
| Experimental pseudonymisation warnings and security note | Under review in #2065, based on #2061. Covers library, CLI, and app entry points; removal is a later, separate change. |
