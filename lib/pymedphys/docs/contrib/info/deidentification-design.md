...
3. **Identifiers.** Consistent replacement of UIDs (D-003) and keyed patient identifiers (D-005), using keys, subject profiles, and any required UID map held by a custodian (D-004). The UID generation scheme is an open decision.
...
| `tps-import` | Research copies for import into non-clinical treatment planning system databases | Basic Profile; Retain Longitudinal Temporal Information with Modified Dates; Retain Patient Characteristics; Retain Device Identity; Clean Descriptors |
...
- These are target option sets. A preset is enabled only once its behaviour is implemented and validated for the documented input scope. Selecting a preset is not evidence of conformance (D-011). The `tps-import` combination of Modified Dates and Retain Device Identity is blocked by D-007; the engine rejects that combination until its conflicting requirements are resolved.
- `tps-import` claims Clean Descriptors only after pooled human review of the retained descriptors (D-009), and writes synthetic birth dates (D-008).
...
- **Decision.**
  ...
  - Where replacement is required, remove trailing NUL and space padding and use one replacement for every occurrence, including nested references, File Meta Information, and retained private elements. Consistency holds within a run and, for incremental export, across runs using the same custodian-held state (D-004).
  - Use UUID-derived `2.25.` UIDs, at most 44 characters long. The generation scheme and UUID version remain open: M2 must establish their standards basis, resistance to re-linking from known source UIDs, and any mapping-state requirements before UID generation is implemented. Do not assume UUIDv8 is permitted by the DICOM reference.
...
- **Rationale.** Unkeyed hashes let anyone who holds the original UIDs re-link records. A keyed deterministic function could avoid a shared map; random UUIDs would require one. `2.25.` UIDs need no registered root, and a scheme that encodes neither a timestamp nor a device identifier avoids disclosing either; an organisation root can identify the institution (E.3.9 Note 2). The alternatives and the unresolved UUIDv8 standards reference are recorded under Open questions.
- **Tests.** Device UID under the Basic Profile and Retain Device Identity tested separately; retained class, coding-scheme, and context-group identifiers; consistent replacements across workers, nested references, File Meta Information, retained private elements, and incremental runs; rejection of unclassified UIDs; and, after writing, both replacements and required retentions. The selected scheme also needs tests for its UUID layout, collision handling, and any mapping-state lifecycle.

### D-004: Keys and subject profiles

- **Decision.**
  - Keys are 256 bits from a cryptographically secure generator. An ephemeral key and any run-only state are discarded after the run; generated identifier consistency is guaranteed only within that run. A project key and its associated state are held by a custodian to preserve generated identifiers across runs, as incremental RT collections need.
  - Use a separate key for each project and recipient, with subject profiles and any UID map isolated per key. Releases using the same key and state are intentionally linkable through their generated identifiers. Different keys separate those identifiers; they do not guarantee that releases cannot be linked through unchanged pixels, geometry, retained attributes, or other information. Assess those channels for each release (D-017).
  ...
  - Project keys, subject profiles, and any UID map required by D-003 are stored only in custodian-controlled locations outside output and QC directories, and never in logs, reports, or the PyMedPhys user configuration.
  ...
  - Incremental export needs the key, subject profile, and any UID map required by the selected scheme. Recipients receive none of this state.
  - Rotating a key starts a new domain for generated identifiers. If a key or required state is lost, earlier exports cannot be extended consistently. A compromised key is treated as disclosure of the crosswalk, and affected releases are reassessed.
- **Rationale.** Medical record numbers and similar identifiers have little entropy, so anyone holding the key can enumerate candidates and re-identify every subject: a project key is equivalent to a crosswalk. Sharing profiles or maps between keys would defeat the intended separation of generated values. Even with isolated state, equal shifted dates or synthetic values can occur by chance, and retained image content remains a separate linkage channel.
- **Tests.** Key-file creation and permissions; refusal to write keys, profiles, or UID maps into output, QC, or configuration locations; stable key identifiers; offsets read from an existing profile after a derivation change; state isolated per key; consistent generated identifiers across incremental releases; and a demonstration that a key enumerates low-entropy identifiers.
...
- **Decision.** Where Retain Longitudinal Temporal Information with Modified Dates is selected, shift the subject-event and radiation-source dates identified in D-007 backwards by a whole number of weeks between 52 and 520, never zero. With a project key, the offset is read from the subject profile (D-004); with an ephemeral key, it is derived for the run. Times are unchanged. Dates found in free text are removed, not shifted. Other temporal roles and conflicting options are handled by D-007. Without the option, the Basic Profile's actions apply.
...
- **Decision.** L2 gives every date, time, and datetime attribute a role. It must satisfy the selected L1 actions and the option's requirements; a more restrictive disclosure policy is not automatically conformant. Where Modified Dates is selected without a conflicting option, each role is handled as follows:
  ...
  - **Device**, the ten calibration, installation, and manufacture attributes listed below: a cleaning method that satisfies C and E.3.6 without exposing the subject's offset remains open. Until one is validated, sequester instances containing these attributes when Modified Dates is selected.
  - **Vocabulary version**, including published Context Group Version (0008,0106) and Template Version (0040,DB06), their local counterparts (0008,0107) and (0040,DB07), and **other**: require a reviewed rule satisfying the effective action and E.3.6 before processing. A published, non-identifying value is a reason to assess retention, not permission to bypass the option's date-modification requirements. Sequester affected instances until such a rule is validated.

  Without Modified Dates, Table E.1-1 applies unchanged.

  In 2026d, eleven attributes are K under Retain Device Identity and C under Modified Dates, the conflicting actions among the target options. Ten are device attributes: Date of Last Calibration (0018,1200), Time of Last Calibration (0018,1201), DateTime of Last Calibration (0018,1202), Calibration DateTime (0018,1203), Date of Manufacture (0018,1204), Date of Installation (0018,1205), Calibration Time (0014,407C), Calibration Date (0014,407E), Date of Last Detector Calibration (0018,700C), and Time of Last Detector Calibration (0018,700E). The eleventh is Beam Hold Transition DateTime. Table E.1-1a requires K values in non-Sequence attributes to remain unchanged; E.3.8 requires their retention, while E.3.6 requires date modification. PS3.15 defines no precedence between these options. Reject this option combination at policy validation, even if a particular input lacks the conflicting attributes, until a standards-supported resolution or an explicitly revised target option set is agreed. The target `tps-import` preset remains unavailable in the meantime.

  Removing, emptying, or dummying a C attribute is not an automatic substitute for cleaning it. An unresolved rule causes rejection or sequestration, not a conformance claim labelled as an interpretation. The generator checks the known conflicts and fails if a new edition adds one; policy validation rejects every unresolved selected-option conflict (D-011).
- **Rationale.** A shifted value whose original is predictable, or a retained true date with a known relationship to a shifted event, can constrain or reveal the subject's offset and hence other shifted dates. MIDI §1.17.9 describes one instance of this, in free text. Device dates are institution-level and may be known. Shifting them per subject would also give one device different dates in different subjects' output, exposing differences between their offsets. Local vocabulary versions are not published, so keeping them is not known to be safe.
- **Consequences.** The first supported Basic Profile release is unaffected by these Modified Dates decisions. Extending coverage needs a conformant rule for each temporal role and a resolution of the conflicting target options. Any future combination that retains Source Serial Number (3008,0105) while shifting Source Strength Reference Date must also assess linkage between subjects treated with the same source and disclosure of differences between their offsets. `public-release` does not retain source serial numbers.
- **Tests.** Every temporal attribute in the pinned dictionary has a role; unresolved option combinations are rejected before writing; unresolved temporal rules sequester affected instances; validated rules satisfy both the table actions and the option text; known-plaintext device and vocabulary cases do not expose the subject's offset through cleaning of non-subject dates; source-to-treatment intervals are preserved; and the generator detects new option conflicts.
...
- **Decision.**
  ...
  - Persist the reference, offset, derivation version, and chosen date in the subject profile (D-004), and reuse the date for every instance and later run. A study that arrives later, whatever its date, never changes the reference.
...
- **Decision.**
  - Validate the effective policy across L1, L2, and L3 before processing. Unresolved conflicts between selected options are rejected; documenting a deviation as an interpretation does not authorise a conformance claim.
  ...
  - The engine does not produce nonconformant output until the open question on it is resolved.
...
- **Decision.**
  ...
  - Key, subject-profile, and any UID-map stores are separate custodian-controlled state and are included in neither.
...
| M2 Primitives | Keys and subject profiles; UID mapping; patient pseudonyms; date offsets; VR and VM validators and dummy values; rule layers and preset composition | The UID scheme is agreed before implementing generation (D-003); unresolved temporal rules and option conflicts block the affected coverage (D-007). Primitives and policies are validated before integration. Hypothesis is added with the first property tests that use it. |
...
- **UID generation (D-003, before implementation in M2).** [PS3.5 B.2](https://dicom.nema.org/medical/dicom/current/output/chtml/part05/sect_B.2.html) describes UUID-derived UIDs through ISO/IEC 9834-8:2014 / [ITU-T X.667 (2012)](https://www.itu.int/rec/T-REC-X.667-201210-I/en). X.667 Table 3 defines versions 1, 3, 4, and 5, and reserves versions 6 to 15. [RFC 9562](https://www.rfc-editor.org/rfc/rfc9562.html) defines version 8 for custom layouts, but that alone does not resolve its use under the DICOM reference.

  | Candidate | Benefit | Work needed before selection |
  | --- | --- | --- |
  | Cryptographically random UUIDv4 with a protected, per-key UID map | Established standards basis; no encoded timestamp; preferred compatibility baseline to evaluate | Coordinated allocation, persistence for incremental export, and recovery handling |
  | Standard UUIDv5 using a domain-separated HMAC-derived token as its name | Deterministic generation without a shared UID map | Specify the namespace and token encoding; review the composition's security and interoperability. Bare UUIDv5 over a source UID permits re-linking; replacing its SHA-1 step with HMAC-SHA256 would not be standard UUIDv5. |
  | HMAC-derived UUIDv8 | Custom layout for deterministic generation | Establish its standards basis under the DICOM reference, as well as the construction's security and interoperability |

  No candidate is approved until its security properties, state requirements, and standards basis are reviewed together.

- **Temporal conformance (D-007, before enabling affected coverage).** Resolve the K/C conflict between Modified Dates and Retain Device Identity against the standard, or explicitly revise the target preset's options and reassess its intended use. Separately establish conformant cleaning rules for device and vocabulary dates under Modified Dates, including preservation of required meaning and protection against disclosure of subject offsets. Until then, reject the conflicting option combination and sequester instances needing unresolved temporal rules. These questions do not block the first release's Basic Profile coverage.
- **Nonconformant output.** Some nonconformant processing will probably be needed, such as acknowledged custom retention or opt-in processing of Private SOP Classes, but which cases to support and how to mark their output is undecided. PS3.15 defines no markers for nonconformant output. A re-identifier sets Patient Identity Removed to NO and removes De-identification Method and its Code Sequence (E.1.2), so NO alongside method values is a combination the standard never produces. Until this is decided, such cases are rejected or sequestered (D-010 and D-011).
