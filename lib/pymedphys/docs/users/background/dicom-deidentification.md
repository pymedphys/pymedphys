...
- Every patient's dates are shifted by the same offset, which is also stored in that configuration, and times are unchanged. Only dates on its keyword list are shifted; RT dates keep their true values. An output file containing both kinds of date therefore reveals the offset, and with it the true dates of every patient pseudonymised on that installation.
