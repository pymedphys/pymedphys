...
- Every patient's dates are shifted by the same offset, which is also stored in that configuration, and times are unchanged. Only dates on its keyword list are shifted; RT dates keep their true values. An output file containing both kinds of date can reveal the offset when the relationship between the original dates is known or can be inferred, for example when both record the same event. Recovering that offset exposes the true values of shifted dates for every patient pseudonymised with that configuration. The presence of both kinds of date alone does not establish their original relationship or uniquely determine the offset.
