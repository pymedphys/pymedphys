...
def propagate_pylintrc():
    with open(ROOT_PYLINT) as f:
        reference_pylint_contents = f.read()

    reference_pylint_contents_with_autogen_warning = (
        "\n".join(AUTOGEN_MESSAGE) + "\n\n" + reference_pylint_contents
    )

    with open(LIBRARY_PYLINT, "w+") as f:
        f.write(reference_pylint_contents_with_autogen_warning)
