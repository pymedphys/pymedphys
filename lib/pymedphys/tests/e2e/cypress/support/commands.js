...
function appearThenDisappear(selector) {
  let start = new Date().getTime();
  cy.get(selector, { timeout: 4000 }).should($el => {
  ...
  cy.get(selector, { timeout: 120000 }).should("not.exist", { timeout: 120000 })
}


Cypress.Commands.add('compute', () => {
  appearThenDisappear(".StatusWidget-enter-done")
})

Cypress.Commands.add('connect', () => {
  appearThenDisappear("[data-testid='stConnectionStatus']")
})
