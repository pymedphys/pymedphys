...
- **Deprecate only when a replacement is usable.** Do not deprecate an interface until its replacement and migration guidance are released. Until then, disclose known limitations or risks in the documentation and with warnings that are not deprecations and make no removal promise, so that a stalled replacement never leaves users on a deprecated tool with nowhere to go.
