...
DATA_DIR = pjoin(HERE, "data", "pseudonymise")
TEST_FILEPATH = pjoin(DATA_DIR, "RP.almost_pseudonymised.dcm")
...
# implemented
TEST_PSEUDONYM_BASENAME = (
    "RP.1.2.246.352.71.5.53598612033.430805.20190416135558_Pseudonymised.dcm"
)
...
def test_anonymise_dataset_and_all_is_anonymised_functions(tmp_path):
    ...
    for keyword in KEYWORDS_FOR_PSEUDONYMS:
    ...
    ds_anon = pseudonymise_dataset(ds)
    ...
    for elem in ds_anon.iterall():
        ...
        ds_single_anon = pseudonymise_dataset(ds_single_non_anon_value)
        _check_is_anonymised_dataset_file_and_dir(
    ...
    ds_anon_blank = pseudonymise_dataset(ds, replace_values=False)
    _check_is_anonymised_dataset_file_and_dir(
    ...
    try:
        ...
        with pytest.raises(ValueError) as e_info:
            pseudonymise_dataset(ds)
        assert str(e_info.value).count(
            "At least one of the non-private tags "
            "within your DICOM file is not within "
            "PyMedPhys copy of the DICOM dictionary."
        )

        ds_anon_delete_unknown = pseudonymise_dataset(ds, delete_unknown_tags=True)
        _check_is_anonymised_dataset_file_and_dir(
        ...
        ds_anon_ignore_unknown = pseudonymise_dataset(ds, delete_unknown_tags=False)
        _check_is_anonymised_dataset_file_and_dir(
    ...
    pseudonymise_dataset(ds, copy_dataset=False)
...
def test_anonymise_file():
    ...
    try:
        ...
        anon_private_filepath = pseudonymise_file(
            TEST_FILEPATH, delete_private_tags=False
        )
        ...
        anon_private_filepath = pseudonymise_file(
            TEST_FILEPATH, delete_private_tags=True
        )
        assert is_anonymised_file(anon_private_filepath, ignore_private_tags=False)

        # Filename is anonymised?
        assert basename(anon_private_filepath) == TEST_PSEUDONYM_BASENAME

        # Deletion of original file
        copyfile(TEST_FILEPATH, temp_filepath)

        anon_filepath_orig = pseudonymise_file(temp_filepath, delete_original_file=True)
        assert is_anonymised_file(anon_filepath_orig)
        assert not exists(temp_filepath)

        # Preservation of filename if desired
        expected_filepath = "{}_Pseudonymised.dcm".format(
            ".".join(TEST_FILEPATH.split(".")[:-1])
        )
        anon_filepath_pres = pseudonymise_file(TEST_FILEPATH, anonymise_filename=False)
...
def test_anonymise_directory(tmp_path):
    ...
    temp_anon_filepath = label_dicom_filepath_as_pseudonymised(temp_filepath)
    try:
        copyfile(TEST_FILEPATH, temp_filepath)
...
def test_pseudonymise_cli(tmp_path):
    ...
    try:
        copyfile(TEST_FILEPATH, temp_filepath)
        temp_anon_filepath = str(tmp_path / TEST_PSEUDONYM_BASENAME)
        # Basic file anonymisation
        assert not is_anonymised_file(temp_filepath)
        assert not exists(temp_anon_filepath)

        anon_file_command = "pymedphys dicom pseudonymise".split() + [temp_filepath]
        try:
        ...
        expected_anon_filepath = label_dicom_filepath_as_pseudonymised(temp_filepath)
        assert not exists(expected_anon_filepath)

        anon_file_pres_command = "pymedphys dicom pseudonymise -f".split() + [
            temp_filepath
        ]
        try:
        ...
        temp_cleared_anon_filepath = str(tmp_path / TEST_PSEUDONYM_BASENAME)

        anon_file_clear_command = "pymedphys dicom pseudonymise -c".split() + [
            temp_filepath
        ]
        try:
        ...
        anon_file_keep_command = (
            "pymedphys dicom pseudonymise".split()
            + [temp_filepath]
            + "-k PatientName".split()
        )
        try:
        ...
        anon_file_private_command = "pymedphys dicom pseudonymise -p".split() + [
            temp_filepath
        ]
        try:
        ...
        anon_dir_command = "pymedphys dicom pseudonymise".split() + [str(tmp_path)]
        try:
...
def test_tags_to_anonymise_in_dicom_dict_baseline(
    ...
    assert set(KEYWORDS_FOR_PSEUDONYMS).issubset(baseline_keywords)
